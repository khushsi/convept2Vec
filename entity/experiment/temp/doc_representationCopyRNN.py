...
if __name__ == '__main__':
    ...
    irfolders=["CopyRNN_allngrams","CopyRNN_NPfiltered","CopyRNNRevised","CopyRNN","TFIDFNPWP1305","TFIDFNPWP1310", "greedy-wiki", "TFIDF1210", "TFIDF1310", "LDA180", "gold_E2v", "wikimergeall", "wikiitalic", "wikiacm", "greedy-acm","CopyRNN"]
    # irfolders = ["TFIDFNPWP1310", "greedy-wiki","TFIDF1210","CopyRNN","gold_E2v","LDA180"]
    combineresults={}
    for ir_directory in irfolders:
        ...
        for filename in os.listdir(ir_directory):
            ...
            if filename.endswith('phrases') and filename.startswith('iir') \
                    ...
                    and not filename.startswith("iir_8") \
                    and not filename.startswith("iir_8_1") \
                    and not filename.startswith("iir 8 6") \
                    and not filename.startswith("iir 8 7") \
                    and not filename.startswith("iir 8") \
                    and not filename.startswith("iir 8 1") \
                    and not filename.startswith("iir_4_5") \
                    and not filename.startswith("iir_4_3") \
                    and not filename.startswith("iir_13_4_1") \
                    and not filename.startswith("iir_13_2_1") \
                    and not filename.startswith("iir 7 1") \
                    and not filename.startswith("iir 7 2") \
                    and not filename.startswith("iir 7 3") \
                    and not filename.startswith("iir 7_1") \
                    and not filename.startswith("iir 7_2") \
                    and not filename.startswith("iir 7_3") \
                    and not filename.startswith("iir 4 5") \
                    and not filename.startswith("iir 4 3") \
                    and not filename.startswith("iir 13 4 1") \
                    and not filename.startswith("iir 13 2 1") :
        ...
        wikiannotations = '../../wikiannotationssmall.csv'
    ...
    # for keyd in combineresults.keys():
    #     result = combineresults[keyd]
    #
    #     list1 =[ x[0] for x in result ]
    #     list2 = [x[1] for x in result]
    #     c = Counter(list1)
    #     print(c.most_common(1))
    #     predictionv = c.most_common(1)[0][0]
    #     idx = np.where(np.array(list1) == predictionv)[0]
    #     print(idx)
    #     rank = np.min(np.array(list2)[idx])
    #     if(predictionv in true[keyd]):
    #         finalrank.append(1/rank)
    #     # print(rank)
    #
    # print(np.sum(finalrank)/totallength)
    ...
    # from tsne import bh_sne
    #
    # vis_data = bh_sne(doc_embed[0][:].astype('float64'))
    # vis_x = vis_data[:, 0]
    # vis_y = vis_data[:, 1]
    # y_data = np.ones(575)
